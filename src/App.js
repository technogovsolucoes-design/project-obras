// GoWorks Manager v0.9.1 — TechnoGov Soluções
...
import { getFirestore, collection, addDoc, updateDoc, deleteDoc, doc, onSnapshot, serverTimestamp, query, orderBy, setDoc, getDoc } from "firebase/firestore";
...
// ─── HOOK PERSISTÊNCIA ────────────────────────────────────────────────────────
const useFireConfig = (user, key, defaultVal) => {
  const [data, setData]     = useState(defaultVal);
  const [loading, setLoad]  = useState(true);
  useEffect(() => {
    if (!user) return;
    const ref = doc(db,"users",user.uid,"config",key);
  ...
  const persist = useCallback(async val => {
    if (!user) return;
    await setDoc(doc(db,"users",user.uid,"config",key),{ value:val, updatedAt:serverTimestamp() });
  }, [user, key]);
  return [data, setData, persist, loading];
};
...
const PBar = ({value,color="#378ADD",height=8}) => (
  <div style={{background:"#f1f5f9",borderRadius:99,height,overflow:"hidden",width:"100%"}}>
    <div style={{width:`${Math.min(100,Math.max(0,Number(value)||0))}%`,background:color,height:"100%",borderRadius:99,transition:"width .5s ease"}}/>
  </div>
);
...
const Btn = ({children,onClick,variant="primary",size="md",disabled=false,full=false}) => {
  const s={primary:{bg:"#185FA5",cl:"white",bd:"none"},secondary:{bg:"#fff",cl:"#334155",bd:"1px solid #cbd5e1"},danger:{bg:"#A32D2D",cl:"white",bd:"none"},success:{bg:"#1D9E75",cl:"white",bd:"none"},orange:{bg:"#C05E14",cl:"white",bd:"none"},green:{bg:BRAND_GREEN,cl:"white",bd:"none"}}[variant];
  return <button onClick={onClick} disabled={disabled} style={{padding:size==="sm"?"5px 12px":"7px 16px",borderRadius:8,background:s.bg,color:s.cl,border:s.bd,cursor:disabled?"not-allowed":"pointer",fontSize:13,fontWeight:500,opacity:disabled?0.5:1,whiteSpace:"nowrap",width:full?"100%":"auto"}}>{children}</button>;
};
const SaveBar = ({onSave,saving,msg}) => (
  <div style={{display:"flex",justifyContent:"flex-end",alignItems:"center",gap:12,padding:"10px 0",borderTop:"1px solid #e2e8f0",marginTop:16}}>
    {msg&&<span style={{fontSize:12,color:msg.includes("Erro")?"#A32D2D":"#1D9E75"}}>{msg}</span>}
    <Btn variant="green" onClick={onSave} disabled={saving}>{saving?"Salvando...":"💾 Salvar alterações"}</Btn>
  </div>
);
...
const LoginScreen = () => {
  ...
  const errMap={"auth/user-not-found":"Usuário não encontrado.","auth/wrong-password":"Senha incorreta.","auth/email-already-in-use":"E-mail já cadastrado.","auth/weak-password":"Senha deve ter pelo menos 6 caracteres.","auth/invalid-email":"E-mail inválido.","auth/too-many-requests":"Muitas tentativas.","auth/invalid-credential":"E-mail ou senha incorretos."};
  const handleLogin=async()=>{ if(!email||!pass){setError("Preencha e-mail e senha.");return;} setLoading(true);setError(""); try{await signInWithEmailAndPassword(auth,email,pass);}catch(e){setError(errMap[e.code]||"Erro ao entrar.");}finally{setLoading(false);} };
  const handleRegister=async()=>{ if(!email||!pass||!name){setError("Preencha nome, e-mail e senha.");return;} setLoading(true);setError(""); try{const c=await createUserWithEmailAndPassword(auth,email,pass);await setDoc(doc(db,"users",c.user.uid,"profile","info"),{nome:name,organizacao:org,criadoEm:serverTimestamp()});}catch(e){setError(errMap[e.code]||"Erro ao cadastrar.");}finally{setLoading(false);} };
  const handleReset=async()=>{ if(!email){setError("Informe o e-mail.");return;} setLoading(true);setError(""); try{await sendPasswordResetEmail(auth,email);setInfo("E-mail de redefinição enviado!");}catch(e){setError(errMap[e.code]||"Erro.");}finally{setLoading(false);} };
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0a2010 0%,#0f3320 40%,#1a5c3a 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:"100%",maxWidth:440}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          ...
          <p style={{margin:0,fontSize:12,color:"rgba(255,255,255,.45)"}}>TechnoGov Soluções • Gestão de Obras Públicas</p>
        ...
        <div style={{background:"rgba(255,255,255,.96)",borderRadius:16,padding:"32px 28px",width:"100%",boxShadow:"0 24px 64px rgba(0,0,0,.4)"}}>
          <p style={{fontSize:15,fontWeight:600,color:"#0f172a",margin:"0 0 18px",borderBottom:`2px solid ${BRAND_GOLD}`,paddingBottom:10}}>
          ...
          {mode==="register"&&(<>
            <div style={{marginBottom:12}}><label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:3}}>Nome completo *</label><input value={name} onChange={e=>setName(e.target.value)} placeholder="Seu nome" style={IS}/></div>
          ...
          <div style={{marginBottom:12}}><label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:3}}>E-mail *</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="seu@email.com.br" style={IS} onKeyDown={e=>e.key==="Enter"&&mode==="login"&&handleLogin()}/></div>
          {mode!=="reset"&&(<div style={{marginBottom:20}}><label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:3}}>Senha *</label><input type="password" value={pass} onChange={e=>setPass(e.target.value)} placeholder="••••••••" style={IS} onKeyDown={e=>e.key==="Enter"&&mode==="login"&&handleLogin()}/></div>)}
          <button onClick={mode==="login"?handleLogin:mode==="register"?handleRegister:handleReset} disabled={loading}
            style={{width:"100%",padding:"10px",borderRadius:8,background:`linear-gradient(135deg,${BRAND_GREEN},#2a9d5c)`,color:"white",border:"none",cursor:loading?"not-allowed":"pointer",fontSize:14,fontWeight:600,opacity:loading?0.7:1,boxShadow:"0 4px 12px rgba(26,107,60,.3)"}}>
        ...
        <p style={{margin:"18px 0 0",fontSize:11,color:"rgba(255,255,255,.25)",textAlign:"center"}}>© 2025 TechnoGov Soluções • v0.9.1</p>
      </div>
    </div>
  );
};
...
const exportExcel = (itens,bdi,obraInfo) => {
  ...
  const tg=itens.reduce((a,i)=>a+Number(i.preco)*Number(i.quantidade)*(1+Number(bdi)/100),0);
  const etapas=[...new Set(itens.map(i=>i.etapa))];
  const meses=[...new Set(itens.map(i=>i.mes))].sort();
  const rows=[[`PLANILHA ORÇAMENTÁRIA — ${obraInfo.nome||"Sem obra"}`],[`BDI: ${bdi}%`,"","","",`Data: ${new Date().toLocaleDateString('pt-BR')}`],[],["Código","Descrição","Un.","Qtd.","Preço Unit.","Total s/BDI","BDI","Total c/BDI","Item da Obra","Etapa"]];
  etapas.forEach(et=>{
    rows.push([`ETAPA: ${et}`]);
    itens.filter(i=>i.etapa===et).forEach(i=>{const s=Number(i.preco)*Number(i.quantidade);const c=s*(1+Number(bdi)/100);rows.push([i.codigo,i.descricao,i.unidade,Number(i.quantidade),Number(i.preco),s,c-s,c,i.itemObra||"",i.etapa]);});
    rows.push([`Subtotal ${et}`,"","","","","","",itens.filter(i=>i.etapa===et).reduce((a,i)=>a+Number(i.preco)*Number(i.quantidade)*(1+Number(bdi)/100),0)]);rows.push([]);
  });
  ...
  etapas.forEach(et=>{const row=[et];let tot=0;meses.forEach(m=>{const v=itens.filter(i=>i.etapa===et&&i.mes===m).reduce((a,i)=>a+Number(i.preco)*Number(i.quantidade)*(1+Number(bdi)/100),0);row.push(v||"");tot+=v;});row.push(tot);row.push(tg>0?`${(tot/tg*100).toFixed(2)}%`:"0%");cR.push(row);});
  const tR=["TOTAL MENSAL"];meses.forEach(m=>tR.push(itens.filter(i=>i.mes===m).reduce((a,i)=>a+Number(i.preco)*Number(i.quantidade)*(1+Number(bdi)/100),0)));tR.push(tg);tR.push("100%");cR.push(tR);
...
const exportPDF=(itens,bdi,obraInfo)=>{
  ...
  ${etapas.map(et=>{let tot=0;const cells=meses.map(m=>{const v=itens.filter(i=>i.etapa===et&&i.mes===m).reduce((a,i)=>a+Number(i.preco)*Number(i.quantidade)*(1+Number(bdi)/100),0);tot+=v;return`<td class="r">${v>0?f(v):"—"}</td>`;}).join("");const pct=tg>0?(tot/tg*100).toFixed(2):0;return`<tr><td>${et}</td>${cells}<td class="r">${f(tot)}</td><td class="r">${pct}%</td></tr>`;}).join("")}
  <tr class="tot"><td>TOTAL</td>${meses.map(m=>{const v=itens.filter(i=>i.mes===m).reduce((a,i)=>a+Number(i.preco)*Number(i.quantidade)*(1+Number(bdi)/100),0);return`<td class="r">${f(v)}</td>`;}).join("")}<td class="r">${f(tg)}</td><td class="r">100%</td></tr></tbody></table>
...
// ─── GEOLOCALIZAÇÃO ───────────────────────────────────────────────────────────
const GeoField = ({value, onChange}) => {
  const [loading, setLoading] = useState(false);
  const [mapOpen, setMapOpen] = useState(false);
  const [search, setSearch]   = useState("");
  const lat = value?.lat || "";
  const lng = value?.lng || "";
  const end = value?.endereco || "";

  const getGPS = () => {
    setLoading(true);
    navigator.geolocation.getCurrentPosition(pos => {
      onChange({ lat: pos.coords.latitude.toFixed(6), lng: pos.coords.longitude.toFixed(6), endereco: end });
      setLoading(false);
    }, () => { alert("Não foi possível obter localização."); setLoading(false); });
  };

  const mapsUrl = lat && lng
    ? `https://www.google.com/maps?q=${lat},${lng}&z=16`
    : `https://www.google.com/maps/search/${encodeURIComponent(end||"obra pública")}`;
  ...
  return (
    <div style={{marginBottom:12}}>
      <label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:3}}>Localização da Obra</label>
      <p style={{fontSize:11,color:"#94a3b8",margin:"0 0 8px",fontStyle:"italic"}}>💡 Informe o endereço e capture as coordenadas GPS para acompanhamento no mapa.</p>
      <Inp label="Endereço / Localização" value={end} onChange={v=>onChange({...value,endereco:v})} placeholder="Ex: Rua das Flores, 100 — Bairro Centro"/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
        <div><label style={{fontSize:11,color:"#64748b",display:"block",marginBottom:3}}>Latitude</label><input value={lat} onChange={e=>onChange({...value,lat:e.target.value})} placeholder="-23.550520" style={{...IS,fontSize:12}}/></div>
        <div><label style={{fontSize:11,color:"#64748b",display:"block",marginBottom:3}}>Longitude</label><input value={lng} onChange={e=>onChange({...value,lng:e.target.value})} placeholder="-46.633308" style={{...IS,fontSize:12}}/></div>
      </div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <Btn size="sm" variant="secondary" onClick={getGPS} disabled={loading}>{loading?"Obtendo GPS...":"📍 Capturar GPS atual"}</Btn>
        {lat&&lng&&(<>
          <a href={mapsUrl} target="_blank" rel="noreferrer" style={{padding:"5px 12px",borderRadius:8,background:"#e6f1fb",color:"#185FA5",fontSize:13,fontWeight:500,textDecoration:"none",display:"inline-flex",alignItems:"center",gap:4}}>🗺 Ver no Google Maps</a>
          <Btn size="sm" variant="secondary" onClick={()=>setMapOpen(s=>!s)}>{mapOpen?"Fechar mapa":"🔍 Mapa inline"}</Btn>
        </>)}
      </div>
      {mapOpen&&embedUrl&&(
        <div style={{marginTop:10,borderRadius:10,overflow:"hidden",border:"1px solid #e2e8f0",height:220}}>
          <iframe title="mapa" src={embedUrl} width="100%" height="220" style={{border:0,display:"block"}} allowFullScreen loading="lazy"/>
        </div>
      )}
      {lat&&lng&&<p style={{fontSize:11,color:"#94a3b8",margin:"6px 0 0"}}>📌 Coordenadas salvas: {lat}, {lng}</p>}
    </div>
  );
};

// ─── BOLETINS ─────────────────────────────────────────────────────────────────
const detectCols=h=>{const hh=h.map(x=>String(x||"").toLowerCase().trim());const find=(...t)=>{for(const x of t){const i=hh.findIndex(v=>v.includes(x));if(i>=0)return i;}return -1;};return{codigo:find("código","codigo","cod","item"),descricao:find("descrição","descricao","desc","denominação","serviço"),unidade:find("unidade","un","unit"),preco:find("custo","preço","preco","valor","price","total")};};

const ModBoletins=({itens,setItens,onSaveItens,saving,saveMsg})=>{
  ...
  const fileRef=useRef(),orcRef=useRef();
  const filtered=useMemo(()=>{const q=busca.toLowerCase();return itens.filter(i=>i.codigo?.toLowerCase().includes(q)||i.descricao?.toLowerCase().includes(q));},[itens,busca]);
  const parseFile=(file,cb)=>{const ext=file.name.split('.').pop().toLowerCase();if(ext==="csv"){Papa.parse(file,{header:false,skipEmptyLines:true,complete:r=>cb(r.data,file.name)});}else{const rd=new FileReader();rd.onload=ev=>{try{const wb=XLSX.read(ev.target.result,{type:"array"});const ws=wb.Sheets[wb.SheetNames[0]];cb(XLSX.utils.sheet_to_json(ws,{header:1,defval:""}),file.name);}catch(e){setLog({type:"error",msg:"Erro: "+e.message});}};rd.readAsArrayBuffer(file);}};
  const handleBoletim=e=>{const file=e.target.files[0];if(!file)return;setImp(true);setLog(null);setMapStep(false);parseFile(file,(data,name)=>{let hIdx=0;for(let i=0;i<Math.min(10,data.length);i++){if(data[i].filter(c=>isNaN(c)&&String(c).trim().length>2).length>2){hIdx=i;break;}}const headers=data[hIdx].map(h=>String(h||"").trim());const rows=data.slice(hIdx+1).filter(r=>r.some(c=>String(c).trim()));const map=detectCols(headers);setRawH(headers);setRawR(rows);setColMap(map);if(map.codigo>=0&&map.descricao>=0)importarBoletim(rows,headers,map,name);else{setMapStep(true);setLog({type:"warn",msg:`Arquivo "${name}" lido. Mapeie as colunas.`});}setImp(false);});e.target.value="";};
  const importarBoletim=(rows,headers,map,filename="")=>{const novos=[];rows.forEach(row=>{const cod=map.codigo>=0?String(row[map.codigo]||"").trim():"";const desc=map.descricao>=0?String(row[map.descricao]||"").trim():"";const un=map.unidade>=0?String(row[map.unidade]||"").trim():"";const pr=map.preco>=0?parseFloat(String(row[map.preco]||"0").replace(/[^\d,.-]/g,"").replace(",",".")):0;if(cod||desc)novos.push({codigo:cod,descricao:desc,unidade:un,preco:Number(pr)||0,fonte,mes,estado});});setItens(prev=>{const ex=prev.filter(p=>!(p.fonte===fonte&&p.mes===mes&&p.estado===estado));return[...ex,...novos];});setLog({type:"success",msg:`${novos.length} itens importados de "${filename}"!`});setMapStep(false);};
  const handleOrcPronto=e=>{const file=e.target.files[0];if(!file)return;setImp(true);setLog(null);parseFile(file,(data,name)=>{let hIdx=0;for(let i=0;i<Math.min(15,data.length);i++){const txt=data[i].map(c=>String(c||"").toLowerCase());if(txt.some(c=>c.includes("descriç")||c.includes("serviço"))&&txt.some(c=>c.includes("unit")||c.includes("preço"))){hIdx=i;break;}}const headers=data[hIdx].map(h=>String(h||"").trim());const rows=data.slice(hIdx+1).filter(r=>r.some(c=>String(c).trim()&&String(c).trim()!=="0"));const map=detectCols(headers);const novos=[];rows.forEach((row,idx)=>{const cod=map.codigo>=0?String(row[map.codigo]||"").trim():`ORC-${idx+1}`;const desc=map.descricao>=0?String(row[map.descricao]||"").trim():"";const un=map.unidade>=0?String(row[map.unidade]||"").trim():"un";const pr=map.preco>=0?parseFloat(String(row[map.preco]||"0").replace(/[^\d,.-]/g,"").replace(",",".")):0;if(desc&&desc.length>3)novos.push({codigo:cod,descricao:desc,unidade:un,preco:Number(pr)||0,fonte:"Planilha importada",mes,estado,_orc:true});});setItens(prev=>[...prev.filter(p=>!p._orc),...novos]);setLog({type:"success",msg:`${novos.length} itens importados da planilha!`});setImp(false);});e.target.value="";};
  const consultarAPI=async()=>{if(!apiCod.trim())return;setApiLoad(true);setApiRes(null);await new Promise(r=>setTimeout(r,800));const local=itens.find(i=>i.codigo===apiCod.trim());setApiRes(local?{...local,origem:"base_local"}:{codigo:apiCod.trim(),descricao:"Item não encontrado na base local.",unidade:"—",preco:0,origem:"nao_encontrado"});setApiLoad(false);};
  ...
  return(
    <div style={{padding:"24px 28px 40px",maxWidth:1000,margin:"0 auto"}}>
      ...
      {tab==="tabela"&&(<div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <SaveBar onSave={onSaveItens} saving={saving} msg={saveMsg}/>
      </div>)}
      {tab==="importar"&&(<div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:700}}>
        <Card><p style={{fontSize:14,fontWeight:500,margin:"0 0 12px"}}>Configurações</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
            ...
            <div><label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:3}}>Estado</label><select value={estado} onChange={e=>setEstado(e.target.value)} style={IS}>{ESTADOS.map(s=><option key={s}>{s}</option>)}</select></div>
        ...
        <Card>
          ...
          <p style={{fontSize:12,color:"#64748b",margin:"0 0 14px"}}>Aceita <strong>Excel (.xlsx)</strong> e <strong>CSV</strong>. Colunas detectadas automaticamente.</p>
          <input ref={fileRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleBoletim} style={{display:"none"}}/>
          <div style={{border:"2px dashed #cbd5e1",borderRadius:10,padding:"28px 20px",textAlign:"center",cursor:"pointer",background:"#f8fafc"}} onClick={()=>fileRef.current?.click()} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f)handleBoletim({target:{files:[f],value:""}});}}>
            <div style={{fontSize:28,marginBottom:6}}>📋</div>
            <p style={{fontSize:13,fontWeight:500,color:"#334155",margin:"0 0 4px"}}>Clique ou arraste o boletim aqui</p>
            <p style={{fontSize:12,color:"#94a3b8",margin:0}}>SINAPI, SICRO, CDHU, SBC — Excel ou CSV</p>
          </div>
          {imp&&<div style={{marginTop:10,display:"flex",alignItems:"center",gap:8,fontSize:13,color:"#185FA5"}}><div style={{width:14,height:14,borderRadius:"50%",border:"2px solid #185FA5",borderTopColor:"transparent",animation:"spin .7s linear infinite"}}/>Processando...</div>}
          {log&&<div style={{marginTop:10,padding:"10px 14px",borderRadius:8,fontSize:13,background:log.type==="success"?"#eaf3de":log.type==="error"?"#FCEBEB":"#FAEEDA",color:log.type==="success"?"#3B6D11":log.type==="error"?"#A32D2D":"#854F0B"}}>{log.msg}</div>}
        </Card>
        {mapStep&&rawH.length>0&&(<Card><p style={{fontSize:14,fontWeight:500,margin:"0 0 10px"}}>Mapeamento de colunas</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {["codigo","descricao","unidade","preco"].map(field=>(<div key={field}><label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:3}}>{field==="preco"?"Preço":field==="codigo"?"Código":field.charAt(0).toUpperCase()+field.slice(1)}</label><select value={colMap?.[field]??""} onChange={e=>setColMap(p=>({...p,[field]:Number(e.target.value)}))} style={IS}><option value={-1}>— Não usar —</option>{rawH.map((h,i)=><option key={i} value={i}>{h||`Coluna ${i+1}`}</option>)}</select></div>))}
          </div>
          <div style={{marginTop:10}}><Btn onClick={()=>importarBoletim(rawR,rawH,colMap)}>Confirmar e importar</Btn></div>
        </Card>)}
        <SaveBar onSave={onSaveItens} saving={saving} msg={saveMsg}/>
        <Card style={{background:"#f8fafc"}}><p style={{fontSize:13,fontWeight:500,margin:"0 0 8px"}}>Onde baixar os boletins oficiais</p>
          {[{nome:"SINAPI",url:"https://www.caixa.gov.br/poder-publico/modernizacao-gestao/sinapi/Paginas/default.aspx",desc:"Portal SINAPI — Caixa Econômica Federal"},{nome:"SICRO",url:"https://www.dnit.gov.br/sicro",desc:"Portal SICRO — DNIT"},{nome:"CDHU",url:"https://www.cdhu.sp.gov.br",desc:"Portal CDHU — Gov. SP"}].map(l=>(
      ...
      {tab==="orc_pronta"&&(<div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:700}}>
        <ABox type="info">Importa planilha orçamentária do engenheiro. O sistema detecta itens, descrições, unidades e preços automaticamente.</ABox>
        <Card>
          ...
          <div style={{border:"2px dashed #185FA5",borderRadius:10,padding:"28px 20px",textAlign:"center",cursor:"pointer",background:"#eff6ff"}} onClick={()=>orcRef.current?.click()}>
            <div style={{fontSize:28,marginBottom:6}}>📊</div>
            <p style={{fontSize:13,fontWeight:500,color:"#185FA5",margin:"0 0 4px"}}>Clique para importar planilha orçamentária</p>
            <p style={{fontSize:12,color:"#94a3b8",margin:0}}>Formato do engenheiro — Excel (.xlsx)</p>
          </div>
          {imp&&<div style={{marginTop:10,display:"flex",alignItems:"center",gap:8,fontSize:13,color:"#185FA5"}}><div style={{width:14,height:14,borderRadius:"50%",border:"2px solid #185FA5",borderTopColor:"transparent",animation:"spin .7s linear infinite"}}/>Processando planilha...</div>}
          {log&&<div style={{marginTop:10,padding:"10px 14px",borderRadius:8,fontSize:13,background:log.type==="success"?"#eaf3de":log.type==="error"?"#FCEBEB":"#FAEEDA",color:log.type==="success"?"#3B6D11":log.type==="error"?"#A32D2D":"#854F0B"}}>{log.msg}</div>}
        </Card>
        <SaveBar onSave={onSaveItens} saving={saving} msg={saveMsg}/>
      </div>)}
      {tab==="api"&&(<div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:600}}>
        ...
        {apiRes&&(<Card style={{borderLeft:apiRes.origem==="nao_encontrado"?"3px solid #FAC775":"3px solid #1D9E75"}}>
          ...
          <p style={{fontSize:13,fontWeight:500,margin:"8px 0 4px"}}>{apiRes.codigo}</p>
        ...
        <ABox type="warning">A Caixa não disponibiliza API pública gratuita para o SINAPI completo. A busca prioriza sua base local importada.</ABox>
      </div>)}
    </div>
  );
};
...
// ─── ORÇAMENTO ────────────────────────────────────────────────────────────────
const CronogramaVis=({itens,bdi})=>{
  const meses=[...new Set(itens.map(i=>i.mes))].sort();
  const etapas=[...new Set(itens.map(i=>i.etapa))];
  const calc=(i)=>Number(i.preco)*Number(i.quantidade)*(1+Number(bdi)/100);
  ...
  const W=520,H=130,PL=40,PR=8,PT=8,PB=22;
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        ...
        <tfoot>
          <tr style={{borderTop:"2px solid #e2e8f0",background:"#f8fafc"}}>
            ...
            {porMes.map(m=><td key={m.mes} style={{padding:"8px 10px",textAlign:"right",fontWeight:600}}>{fmtBRL(m.val)}</td>)}
...
const ModOrcamento=({obras,boletimItens,user})=>{
  ...
  // Calcula totais com Number() garantido
  const calc=(i)=>Number(i.preco)*Number(i.quantidade)*(1+Number(bdi)/100);
  const tSemBdi=itensSel.reduce((a,i)=>a+Number(i.preco)*Number(i.quantidade),0);
  const tComBdi=itensSel.reduce((a,i)=>a+calc(i),0);
  const tBdi=tComBdi-tSemBdi;

  const filtrados=useMemo(()=>{const q=busca.toLowerCase();return boletimItens.filter(i=>(fonte==="Todas"||i.fonte===fonte)&&(i.codigo?.toLowerCase().includes(q)||i.descricao?.toLowerCase().includes(q)));},[boletimItens,busca,fonte]);
  const isSel=cod=>itensSel.find(i=>i.codigo===cod);
  const toggleItem=item=>{if(isSel(item.codigo))setItensSel(prev=>prev.filter(i=>i.codigo!==item.codigo));else setItensSel(prev=>[...prev,{...item,quantidade:1,etapa:etapasObra[0]||"Geral",mes:MESES_REF[0],itemObra:itensCad[0]||""}]);};
  ...
  const selectAll=()=>{const nao=filtrados.filter(i=>!isSel(i.codigo));setItensSel(prev=>[...prev,...nao.map(item=>({...item,quantidade:1,etapa:etapasObra[0]||"Geral",mes:MESES_REF[0],itemObra:itensCad[0]||""}))]);};

  const handleGerar=()=>{if(itensSel.length===0)return;setGerado(true);setTab("orcamento");};
  const handleSalvar=async()=>{
    if(!user||itensSel.length===0)return;
    setSaving(true);
    try{await addDoc(collection(db,"users",user.uid,"orcamentos"),{obraId,obraNome:obra.nome||"Independente",bdi,itens:itensSel,total:tComBdi,criadoEm:serverTimestamp()});setSaveMsg("✅ Orçamento salvo!");}
  ...
  return(
    <div style={{padding:"24px 28px 40px",maxWidth:1100,margin:"0 auto"}}>
      ...
      {saveMsg&&<div style={{position:"fixed",bottom:20,right:20,background:saveMsg.includes("Erro")?"#A32D2D":"#1D9E75",color:"white",padding:"10px 18px",borderRadius:8,fontSize:13,zIndex:99,boxShadow:"0 4px 12px rgba(0,0,0,.2)"}}>{saveMsg}</div>}
      <Card style={{marginBottom:20}}>
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:16,alignItems:"end"}}>
          ...
          <div><label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:3}}>Resumo financeiro</label>
            <div style={{fontSize:12,color:"#64748b",lineHeight:1.9}}>S/ BDI: <strong>{fmtBRL(tSemBdi)}</strong><br/>BDI ({bdi}%): <strong style={{color:"#854F0B"}}>{fmtBRL(tBdi)}</strong><br/>C/ BDI: <strong style={{color:"#185FA5"}}>{fmtBRL(tComBdi)}</strong></div></div>
        </div>
      </Card>
      <div style={{display:"flex",gap:0,borderBottom:"1px solid #e2e8f0",marginBottom:20,overflowX:"auto"}}>
      ...
      {tab==="config"&&(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,maxWidth:800}}>
        <Card><p style={{fontSize:14,fontWeight:500,margin:"0 0 12px"}}>Etapas do Orçamento</p>
          <div style={{display:"flex",gap:8,marginBottom:10}}><input value={novaEtapa} onChange={e=>setNovaEtapa(e.target.value)} placeholder="Nova etapa..." style={{...IS,flex:1}}/><Btn size="sm" onClick={()=>{if(novaEtapa.trim()){setEtapasCad(p=>[...p,novaEtapa.trim()]);setNovaEtapa("");}}}>+</Btn></div>
          {etapasObra.map((et,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",background:"#f8fafc",borderRadius:6,fontSize:13,marginBottom:4}}>
            <span>{et}</span>{etapasCad.includes(et)&&<button onClick={()=>setEtapasCad(p=>p.filter(e=>e!==et))} style={{background:"none",border:"none",cursor:"pointer",color:"#A32D2D",fontSize:11}}>✕</button>}
          </div>))}
        </Card>
        <Card><p style={{fontSize:14,fontWeight:500,margin:"0 0 12px"}}>Itens da Obra</p>
          <div style={{display:"flex",gap:8,marginBottom:10}}><input value={novoItem} onChange={e=>setNovoItem(e.target.value)} placeholder="Novo item..." style={{...IS,flex:1}}/><Btn size="sm" onClick={()=>{if(novoItem.trim()){setItensCad(p=>[...p,novoItem.trim()]);setNovoItem("");}}}>+</Btn></div>
          {itensCad.map((it,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",background:"#f8fafc",borderRadius:6,fontSize:13,marginBottom:4}}>
            <span>{it}</span><button onClick={()=>setItensCad(p=>p.filter(e=>e!==it))} style={{background:"none",border:"none",cursor:"pointer",color:"#A32D2D",fontSize:11}}>✕</button>
          </div>))}
        </Card>
      </div>)}
      ...
      {tab==="selecao"&&(<div style={{display:"flex",flexDirection:"column",gap:16}}>
        <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <select value={fonte} onChange={e=>setFonte(e.target.value)} style={{...IS,width:"auto",fontSize:13}}>{["Todas","SINAPI","CDHU","SICRO","SBC","Planilha importada"].map(f=><option key={f}>{f}</option>)}</select>
          ...
          <span style={{fontSize:12,color:"#64748b"}}>{filtrados.length} disponíveis</span>
        </div>
        <Card style={{padding:0,overflow:"hidden"}}>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{background:"#f8fafc"}}>
                ...
                {["Código","Descrição","Un.","Preço unit."].map(h=><th key={h} style={{padding:"10px 12px",textAlign:"left",fontSize:11,fontWeight:500,color:"#64748b",whiteSpace:"nowrap"}}>{h}</th>)}
              ...
              <tbody>
                {filtrados.map((item,idx)=>{
                  ...
                  const totalItem=sel?fmtBRL(Number(sel.preco)*Number(sel.quantidade)*(1+Number(bdi)/100)):null;
                  return(<tr key={idx} style={{borderTop:"1px solid #f1f5f9",background:sel?"#eff6ff":"#fff"}}>
                    ...
                    <td style={{padding:"6px 8px",background:"#f8fbff"}}>
                      {sel?<select value={sel.itemObra||""} onChange={e=>updateSel(item.codigo,"itemObra",e.target.value)} style={{...IS,minWidth:110,padding:"5px 8px",fontSize:12}}><option value="">—</option>{itensCad.map(it=><option key={it} value={it}>{it}</option>)}</select>:<span style={{fontSize:11,color:"#cbd5e1",display:"block",textAlign:"center"}}>—</span>}
                    </td>
                    <td style={{padding:"6px 8px",background:"#f8fbff"}}>
                      {sel?<select value={sel.etapa} onChange={e=>updateSel(item.codigo,"etapa",e.target.value)} style={{...IS,minWidth:120,padding:"5px 8px",fontSize:12}}>{etapasObra.map(et=><option key={et} value={et}>{et}</option>)}</select>:<span style={{fontSize:11,color:"#cbd5e1",display:"block",textAlign:"center"}}>—</span>}
                    </td>
                    <td style={{padding:"6px 8px",background:"#f8fbff"}}>
                      {sel?<select value={sel.mes} onChange={e=>updateSel(item.codigo,"mes",e.target.value)} style={{...IS,minWidth:90,padding:"5px 8px",fontSize:12}}>{MESES_REF.map(m=><option key={m} value={m}>{m}</option>)}</select>:<span style={{fontSize:11,color:"#cbd5e1",display:"block",textAlign:"center"}}>—</span>}
                    </td>
                    <td style={{padding:"10px 12px",textAlign:"right",fontSize:12,fontWeight:500,background:"#f8fbff",whiteSpace:"nowrap"}}>
                      {sel?<span style={{color:"#185FA5"}}>{totalItem}</span>:<span style={{color:"#cbd5e1"}}>—</span>}
                    </td>
                  </tr>);})}
              </tbody>
            </table>
      ...
      {/* PLANILHA */}
      {tab==="orcamento"&&(!gerado?<Card><p style={{textAlign:"center",color:"#64748b",fontSize:13,padding:"24px 0"}}>Selecione os itens e clique em "Gerar".</p></Card>:(
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
            <div><p style={{margin:0,fontSize:14,fontWeight:500}}>{obra.nome||"Orçamento independente"}</p><p style={{margin:0,fontSize:12,color:"#64748b"}}>BDI: {bdi}% • {itensSel.length} itens • {new Date().toLocaleDateString('pt-BR')}</p></div>
          ...
          {[...new Set(itensSel.map(i=>i.etapa))].map(et=>{
            ...
            return(<div key={et}>
              ...
              <Card style={{borderRadius:"0 0 8px 8px",padding:0,overflow:"hidden",borderTop:"none"}}>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  <thead><tr style={{background:"#f8fafc"}}>{["Código","Descrição","Un.","Qtd.","Preço Unit.","S/BDI","BDI","C/BDI","Item",""].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",fontSize:11,fontWeight:500,color:"#64748b",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
                  <tbody>
                    {ei.map((i,idx)=>{const s=Number(i.preco)*Number(i.quantidade);const c=s*(1+Number(bdi)/100);return(
                      <tr key={idx} style={{borderTop:"1px solid #f1f5f9"}}>
                        <td style={{padding:"8px 10px",fontSize:11,color:"#185FA5",fontWeight:500,whiteSpace:"nowrap"}}>{i.codigo}</td>
                        <td style={{padding:"8px 10px",fontSize:11,color:"#0f172a",maxWidth:240}}><input value={i.descricao} onChange={e=>updateSel(i.codigo,"descricao",e.target.value)} style={{...IS,padding:"3px 6px",fontSize:11,border:"none",background:"transparent"}}/></td>
                        <td style={{padding:"8px 10px",fontSize:11,color:"#64748b",textAlign:"center"}}>{i.unidade}</td>
                        <td style={{padding:"8px 10px",fontSize:11,textAlign:"right"}}><input type="number" value={i.quantidade} onChange={e=>updateSel(i.codigo,"quantidade",e.target.value)} style={{...IS,width:60,padding:"3px 6px",fontSize:11,textAlign:"right"}}/></td>
                  ...
                  <tfoot><tr style={{background:"#f1f5f9",borderTop:"2px solid #e2e8f0"}}><td colSpan={7} style={{padding:"8px 10px",fontSize:12,fontWeight:600,color:"#334155"}}>Subtotal {et}</td><td style={{padding:"8px 10px",fontSize:13,fontWeight:700,textAlign:"right",color:"#0f172a",whiteSpace:"nowrap"}}>{fmtBRL(totEt)}</td><td colSpan={2}></td></tr></tfoot>
                </table>
              </Card>
            </div>);})}
          <div style={{background:"#0f172a",borderRadius:10,padding:"14px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
      ...
      {/* CRONOGRAMA */}
      {tab==="cronograma"&&(!gerado?<Card><p style={{textAlign:"center",color:"#64748b",fontSize:13,padding:"24px 0"}}>Selecione os itens e clique em "Gerar".</p></Card>:(
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
            <div><p style={{margin:0,fontSize:14,fontWeight:500}}>Cronograma Físico-Financeiro</p><p style={{margin:0,fontSize:12,color:"#64748b"}}>{obra.nome||"Independente"} • BDI {bdi}% • Total: {fmtBRL(tComBdi)}</p></div>
            <div style={{display:"flex",gap:8}}>
              <Btn variant="secondary" size="sm" onClick={()=>exportExcel(itensSel,bdi,obra)}>⬇ Excel</Btn>
              <Btn variant="orange" size="sm" onClick={()=>exportPDF(itensSel,bdi,obra)}>🖨 PDF</Btn>
            </div>
          </div>
          <Card><CronogramaVis itens={itensSel} bdi={bdi}/></Card>
        </div>
      ))}
    </div>
  );
...
// ─── MÓDULOS DO PROCESSO ──────────────────────────────────────────────────────
const DICAS={
  nome:"Ex: Construção do Centro Cultural Municipal, Reforma da UBS Jardim das Flores",
  objeto_resumido:"Descreva o que será feito, onde e qual a finalidade pública.",
  descricao_necessidade:"Aponte o problema público atual que justifica a obra.",
  comparativo_solucoes:"Analise alternativas e justifique a solução escolhida.",
  requisitos_contratacao:"Cite normas ABNT, NR-18, CONAMA aplicáveis.",
  estimativas_quantitativas:"Descreva como os quantitativos foram calculados.",
  impacto_ambiental:"PGRCC, destinação de resíduos, supressão vegetal.",
  prevencao_paralisacoes:"Riscos mapeados: desapropriação, interferências, projetos incompletos.",
  posicionamento_conclusivo:"Declaração formal dos servidores sobre viabilidade técnica — Art. 18 §1º XIII.",
};
...
const ModIdentificacao=({data,update,setData})=>{
  ...
  const addEtapa=()=>{if(!novaEtapa.nome.trim())return;const nova={...novaEtapa,id:Date.now(),peso:Number(novaEtapa.peso)||0,status:"nao_iniciada",inicio_real:null,fim_real:null};setData(p=>({...p,execucao:{...p.execucao,etapas:[...(p.execucao?.etapas||[]),nova]}}));setNovaEtapa({nome:"",peso:0,inicio_plan:"",fim_plan:""});};
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      <Sel label="Status Atual" value={data.status} onChange={v=>update(null,"status",v)} options={STATUS_LIST}/>
      <Inp label="Orçamento Estimado (R$)" type="number" value={data.orcamento_estimado} onChange={v=>update(null,"orcamento_estimado",v)} prefix="R$" tip="Preencha após concluir ETP e projeto básico."/>
    </div>
    ...
    {/* Geolocalização */}
    <Card>
      <GeoField value={data.localizacao||{}} onChange={v=>update(null,"localizacao",v)}/>
    </Card>
    {/* Etapas */}
    <Card>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div><p style={{fontSize:13,fontWeight:600,margin:0}}>Etapas da Obra</p><p style={{fontSize:11,color:"#94a3b8",margin:"2px 0 0"}}>Usadas no cronograma físico-financeiro. Peso total: <strong style={{color:totalPeso===100?BRAND_GREEN:totalPeso>100?"#A32D2D":"#854F0B"}}>{totalPeso}%</strong></p></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"3fr 1fr 1fr 1fr auto",gap:8,marginBottom:10,alignItems:"end"}}>
        <div><label style={{fontSize:11,color:"#64748b",display:"block",marginBottom:3}}>Nome da etapa *</label><input value={novaEtapa.nome} onChange={e=>setNovaEtapa(p=>({...p,nome:e.target.value}))} placeholder="Ex: Fundações" style={{...IS,fontSize:12}} onKeyDown={e=>e.key==="Enter"&&addEtapa()}/></div>
        ...
        <button onClick={addEtapa} style={{padding:"7px 14px",borderRadius:8,background:`linear-gradient(135deg,${BRAND_GREEN},#2a9d5c)`,color:"white",border:"none",cursor:"pointer",fontSize:13,fontWeight:600,marginTop:18,whiteSpace:"nowrap"}}>+ Add</button>
      </div>
      {etapas.length===0?<p style={{fontSize:12,color:"#94a3b8",textAlign:"center",padding:"12px 0",margin:0}}>Nenhuma etapa cadastrada.</p>
      :<div style={{display:"flex",flexDirection:"column",gap:5}}>
        {etapas.map((e,i)=>(
          <div key={e.id} style={{display:"grid",gridTemplateColumns:"3fr 1fr 1fr 1fr auto",gap:8,alignItems:"center",padding:"8px 10px",background:"#f8fafc",borderRadius:8,border:"1px solid #e2e8f0"}}>
            ...
            <button onClick={()=>delEtapa(e.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#A32D2D",fontSize:14}}>✕</button>
          </div>))}
      </div>}
    </Card>
  </div>);
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    <Card><p style={{fontSize:13,fontWeight:500,margin:"0 0 12px"}}>Configuração</p>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <Sel label="Base de Custos" value={etp.base_custos} onChange={v=>u("base_custos",v)} options={BASES_CUSTO} tip="SINAPI para edificações. SICRO para rodovias. CDHU para obras do Estado de SP."/>
      </div>
      ...
      <Inp label="Estimativa de Valor (R$)" type="number" value={etp.estimativa_valor} onChange={v=>u("estimativa_valor",v)} prefix="R$" tip="Deve ser sigiloso até abertura das propostas (Art. 24)."/>
    </Card>
    <Card><p style={{fontSize:13,fontWeight:500,margin:"0 0 12px"}}>1. Necessidade e Comparação</p>
    ...
    <Card><p style={{fontSize:13,fontWeight:500,margin:"0 0 12px"}}>2. Requisitos e Quantitativos</p>
      ...
      <Txt label="Estimativas Quantitativas e Memória (Art. 18, §1º, IV)" value={etp.estimativas_quantitativas} onChange={v=>u("estimativas_quantitativas",v)} rows={3} tip={DICAS.estimativas_quantitativas}/>
    </Card>
...
const ModGeral=({data,update})=>{
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:16}}>
    <Card><p style={{fontSize:13,fontWeight:500,margin:"0 0 12px"}}>Identificação Complementar</p>
      ...
      <Inp label="Orçamento Estimado (R$)" type="number" value={data.orcamento_estimado} onChange={v=>update(null,"orcamento_estimado",v)} prefix="R$"/>
    </Card>
    <Card><p style={{fontSize:13,fontWeight:500,margin:"0 0 12px"}}>Convênios e Repasses</p>
      <Sel label="Obra Conveniada?" value={conv.tem} onChange={v=>update("convenio","tem",v)} options={["nao","sim"]} tip="Informe se há transferência de recursos de outro ente (Estado, União, FNDE, etc.)."/>
      {conv.tem==="sim"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <Inp label="Número do Convênio / SICONV" value={conv.numero} onChange={v=>update("convenio","numero",v)} tip="Ex: TC-2024/00123"/>
        <Inp label="Órgão Concedente" value={conv.orgao} onChange={v=>update("convenio","orgao",v)} tip="Ex: Ministério da Saúde, Caixa, Gov. SP"/>
        <Inp label="Valor do Repasse (R$)" type="number" value={conv.valor_repasse} onChange={v=>update("convenio","valor_repasse",v)} prefix="R$"/>
        <Inp label="Contrapartida Municipal (R$)" type="number" value={conv.valor_contrapartida} onChange={v=>update("convenio","valor_contrapartida",v)} prefix="R$" tip="Percentual obrigatório conforme o convênio."/>
      </div>}
    </Card>
  </div>);
};
const ModEngenharia=({data,update})=>{
  ...
  return(<Card><p style={{fontSize:13,fontWeight:500,margin:"0 0 12px"}}>Dados de Engenharia</p>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      <Inp label="ART / RRT de Projeto" value={eng.art_rrt} onChange={v=>update("engenharia","art_rrt",v)} tip="Número da ART (CREA) ou RRT (CAU). Obrigatório para projetos de engenharia."/>
      <Inp label="Data Base do Orçamento" type="date" value={eng.data_base_orcamento} onChange={v=>update("engenharia","data_base_orcamento",v)} tip="Mês do boletim de preços utilizado. Importante para cálculo de reajuste."/>
      <Inp label="Prazo de Execução (dias)" type="number" value={eng.prazo_execucao_dias} onChange={v=>update("engenharia","prazo_execucao_dias",v)} tip="Dias corridos. Considere sazonalidade e disponibilidade de materiais."/>
    </div>
  </Card>);
};
const ModLicitacao=({data,update})=>{
  ...
  return(<Card><p style={{fontSize:13,fontWeight:500,margin:"0 0 12px"}}>Dados da Licitação</p>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      <Inp label="Nº Processo Administrativo" value={lic.numero_processo} onChange={v=>update("licitacao","numero_processo",v)} tip="Ex: ADM-001/2025. Gerado pelo protocolo da prefeitura."/>
      <Inp label="Nº do Edital" value={lic.numero_edital} onChange={v=>update("licitacao","numero_edital",v)} tip="Ex: CC-001/2025, PE-003/2025."/>
      <Sel label="Modalidade (Lei 14.133/2021)" value={lic.modalidade} onChange={v=>update("licitacao","modalidade",v)} options={MODALIDADES} tip="Obras acima de R$6M → Concorrência. Abaixo → Pregão ou Dispensa."/>
      <Sel label="Regime de Execução" value={lic.regime} onChange={v=>update("licitacao","regime",v)} options={REGIMES} tip="Preço Global: valor fechado. Preço Unitário: por item medido."/>
      <Inp label="Data de Abertura das Propostas" type="date" value={lic.data_abertura} onChange={v=>update("licitacao","data_abertura",v)} tip="Prazo mínimo: 25 dias úteis (Concorrência), 8 dias úteis (Pregão)."/>
    </div>
  </Card>);
};
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:16}}>
    <Card><p style={{fontSize:13,fontWeight:500,margin:"0 0 12px"}}>Dados Contratuais</p>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <Inp label="Nº do Contrato" value={ct.numero} onChange={v=>update("contrato","numero",v)} tip="Ex: CT-001/2025. Numeração da PGM/Jurídico."/>
        <Inp label="Data de Assinatura" type="date" value={ct.data_assinatura} onChange={v=>update("contrato","data_assinatura",v)}/>
        <Inp label="Valor Inicial (R$)" type="number" value={ct.valor_inicial} onChange={v=>update("contrato","valor_inicial",v)} prefix="R$" tip="Valor adjudicado. Pode diferir do orçamento estimado."/>
      </div>
    </Card>
    <Card><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <div><p style={{fontSize:13,fontWeight:500,margin:0}}>Termos Aditivos</p><p style={{fontSize:11,color:"#94a3b8",margin:"2px 0 0"}}>Art. 124 Lei 14.133/2021 — Limite: 25% acréscimo obras, 50% reformas</p></div>
    ...
    :ct.aditivos.map((a,i)=><div key={a.id} style={{background:"#f8fafc",borderRadius:8,padding:12,marginBottom:8}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        ...
        <Inp label="Justificativa Técnica" value={a.justificativa} onChange={v=>updAd(a.id,"justificativa",v)} tip="Fundamento legal: Art. 124, I a IV da Lei 14.133/2021."/>
...
const CSvg=({medicoes})=>{
  ...
  let acc=0;const ex=m.map((_,i)=>{const v=medicoes[i];if(v){acc+=Number(v.percentual_periodo)||0;return Math.round(acc);}return null;});
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:0}}>
    ...
    {tab==="dashboard"&&(<div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10}}>
        ...
        <MCard label="Saldo" value={fmtBRL(valCt-totFin)} accent={BRAND_GOLD}/>
      </div>
      <Card>
    ...
    {tab==="cronograma"&&(<div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {showEF&&(<div style={{background:"#f8fafc",borderRadius:10,padding:14,display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:8}}>
        ...
        <button onClick={addEt} style={{gridColumn:"1/-1",padding:8,borderRadius:8,background:BRAND_GREEN,color:"white",border:"none",cursor:"pointer",fontSize:13}}>Adicionar etapa</button>
      </div>)}
      {eta.length===0&&<p style={{fontSize:13,color:"#64748b"}}>Nenhuma etapa. Cadastre na aba Identificação.</p>}
    ...
    {tab==="medicoes"&&(<div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {showMF&&(<div style={{background:"#f8fafc",borderRadius:10,padding:14,display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        ...
        <button onClick={addMed} style={{gridColumn:"1/-1",padding:8,borderRadius:8,background:BRAND_GREEN,color:"white",border:"none",cursor:"pointer",fontSize:13}}>Salvar medição</button>
      </div>)}
      {med.length===0&&<p style={{fontSize:13,color:"#64748b"}}>Nenhuma medição registrada.</p>}
      {[...med].reverse().map(m=>(<div key={m.id} style={{background:"#fff",border:`1px solid ${m.aprovada?"#C0DD97":"#e2e8f0"}`,borderLeft:`3px solid ${m.aprovada?BRAND_GREEN:"#378ADD"}`,borderRadius:10,padding:"12px 16px"}}>
    ...
    {tab==="diario"&&(<div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {showDF&&(<div style={{background:"#f8fafc",borderRadius:10,padding:14,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
        ...
        <button onClick={addR} style={{gridColumn:"1/-1",padding:8,borderRadius:8,background:BRAND_GREEN,color:"white",border:"none",cursor:"pointer",fontSize:13}}>Salvar registro</button>
      </div>)}
    ...
    {tab==="galeria"&&(<div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{background:"#f8fafc",borderRadius:10,padding:14,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        ...
        <div style={{gridColumn:"1/-1"}}><input ref={fileRef} type="file" accept="image/*" onChange={handleF} style={{display:"none"}}/><button onClick={()=>fileRef.current?.click()} style={{padding:"7px 16px",borderRadius:8,background:BRAND_GREEN,color:"white",border:"none",cursor:"pointer",fontSize:13}}>Enviar foto com geolocalização</button><span style={{fontSize:11,color:"#64748b",marginLeft:10}}>GPS automático via navegador</span></div>
      </div>
...
const ProcessoForm=({obra,onSave,onBack,onDelete,saving})=>{
  ...
  const handleSave=async()=>{try{await onSave(data);setToast({type:"success",msg:"💾 Processo salvo no Firebase!"});}catch(e){setToast({type:"error",msg:"Erro: "+e.message});}setTimeout(()=>setToast(null),3000);};
  return(<div style={{display:"flex",flexDirection:"column",height:"100%",background:"#f8fafc"}}>
    <div style={{background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"12px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:10}}>
      ...
      <div style={{display:"flex",gap:8}}>{obra?.id&&<Btn variant="danger" size="sm" onClick={()=>onDelete(obra.id)}>Excluir</Btn>}<Btn variant="green" onClick={handleSave} disabled={saving}>{saving?"Salvando...":"💾 Salvar"}</Btn></div>
    </div>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"20px"}}>
      <div style={{maxWidth:820,margin:"0 auto",paddingBottom:40}}>
        ...
        {/* SaveBar em todas as abas */}
        {tab!=="exec"&&<SaveBar onSave={handleSave} saving={saving} msg={null}/>}
      </div>
    </div>
  </div>);
};
...
const Painel=({obras,onCreate,onSelect})=>{
  ...
  return(<div style={{paddingBottom:40}}>
    <div style={{background:"linear-gradient(135deg,#0a2010 0%,#0f3320 50%,#1a5c3a 100%)",padding:"28px 28px 32px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
        <LogoTG size={50}/>
        <div><h1 style={{margin:0,fontSize:22,fontWeight:800,color:"white",letterSpacing:"-0.5px"}}>Go<span style={{color:BRAND_GOLD}}>Works</span> Manager</h1><p style={{margin:"2px 0 0",fontSize:11,color:"rgba(255,255,255,.45)"}}>TechnoGov Soluções • Gestão Inteligente de Obras Públicas • v0.9.1</p></div>
      </div>
            ...
            <tbody>
              {obras.map(o=>(<tr key={o.id} style={{borderTop:"1px solid #e2e8f0"}}>
                ...
                <td style={{padding:"12px 16px",fontSize:11,color:"#64748b",whiteSpace:"nowrap"}}>
                  {o.localizacao?.lat&&o.localizacao?.lng
                    ?<a href={`https://www.google.com/maps?q=${o.localizacao.lat},${o.localizacao.lng}`} target="_blank" rel="noreferrer" style={{color:"#185FA5",textDecoration:"none",fontSize:11}}>📍 Ver no mapa</a>
...
export default function App() {
  ...
  // Boletins persistidos no Firebase
  const [boletimItens,setBI,saveBI,biLoading] = useFireConfig(user,"boletimItens",SINAPI_DEMO);
  const [savingBI,setSavingBI] = useState(false);
  const [saveMsgBI,setSaveMsgBI] = useState(null);

  const handleSaveBoletins = async () => {
    setSavingBI(true);
    try { await saveBI(boletimItens); setSaveMsgBI("✅ Base de boletins salva!"); }
    catch(e) { setSaveMsgBI("Erro: "+e.message); }
    finally { setSavingBI(false); setTimeout(()=>setSaveMsgBI(null),3000); }
  };
  ...
  if(authLoading||biLoading)return(<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#f8fafc",flexDirection:"column",gap:12}}>
    <div style={{width:36,height:36,borderRadius:"50%",border:`3px solid ${BRAND_GREEN}`,borderTopColor:"transparent",animation:"spin .8s linear infinite"}}/>
    <p style={{fontSize:13,color:"#64748b",margin:0}}>Carregando GoWorks Manager...</p>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",fontFamily:"system-ui,sans-serif",background:"#f8fafc",overflow:"hidden"}}>
      ...
      {/* Topbar mobile */}
      <div className="gw-tb" style={{display:"none",background:"#0a2010",padding:"12px 16px",alignItems:"center",justifyContent:"space-between",flexShrink:0,borderBottom:`1px solid rgba(201,168,76,.15)`}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}><LogoTG size={28}/><span style={{fontWeight:700,fontSize:14,color:"white"}}>Go<span style={{color:BRAND_GOLD}}>Works</span></span></div>
        <button onClick={()=>setMenu(s=>!s)} style={{background:"none",border:"none",cursor:"pointer",color:"white",fontSize:22}}>{menuOpen?"✕":"☰"}</button>
      </div>
      {menuOpen&&(<div style={{background:"#0a2010",padding:"12px 10px",flexShrink:0,borderBottom:`1px solid rgba(201,168,76,.15)`}}>
        {NAV.map(n=><button key={n.id} onClick={()=>handleNav(n.id)} style={{display:"block",width:"100%",textAlign:"left",padding:"10px 12px",borderRadius:6,background:view===n.id?"rgba(201,168,76,.15)":"none",border:"none",cursor:"pointer",fontSize:14,color:view===n.id?BRAND_GOLD:"white",marginBottom:4}}>{n.icon} {n.label}</button>)}
        <button onClick={()=>signOut(auth)} style={{display:"block",width:"100%",textAlign:"left",padding:"10px 12px",borderRadius:6,background:"none",border:"none",cursor:"pointer",fontSize:13,color:"rgba(255,255,255,.4)",marginTop:8}}>⎋ Sair</button>
      </div>)}
      ...
      <div className="gw-ly" style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <aside className="gw-sb" style={{display:"none",width:232,background:"linear-gradient(180deg,#0a2010 0%,#0f3320 60%,#0a2010 100%)",flexDirection:"column",flexShrink:0,borderRight:`1px solid rgba(201,168,76,.15)`}}>
          <div style={{padding:"20px 18px 16px",borderBottom:`1px solid rgba(201,168,76,.15)`}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}><LogoTG size={36}/><div><p style={{margin:0,fontSize:14,fontWeight:800,color:"white"}}>Go<span style={{color:BRAND_GOLD}}>Works</span></p><p style={{margin:0,fontSize:9,color:"rgba(255,255,255,.4)",letterSpacing:"0.5px"}}>MANAGER v0.9.1</p></div></div>
        ...
        <main style={{flex:1,overflowY:view==="form"?"hidden":"auto",display:"flex",flexDirection:"column"}}>
          ...
          {view==="boletins"&&<ModBoletins itens={boletimItens} setItens={setBI} onSaveItens={handleSaveBoletins} saving={savingBI} saveMsg={saveMsgBI}/>}
